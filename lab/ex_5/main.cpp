...
int main(int argc, char const *argv[])
{
    ...
    // EDD
    std::cout << "\nEDD LIST scheduling" << std::endl;
    LIST_rule(&jobs, &resources, EDD_rule);
    Simulation_P(&resources, 0);
    obj_f = Evaluate(&jobs);
    print_obj_func(obj_f);
    if (table)
    ...
    // LPT
    std::cout << "\nLPT LIST scheduling" << std::endl;
    LIST_rule(&jobs, &resources, LPT_rule);
    Simulation_P(&resources, 0);
    obj_f = Evaluate(&jobs);
    print_obj_func(obj_f);
    if (table)
               ...
               void rule(std::vector<std::shared_ptr<Job>> *))
{
    ...
    for (auto &job : *jobs)
    {
        auto selected_resource = resources->front();
        for (auto &resource : *resources)
        {
            if (resource->c() < selected_resource->c())
            {
                selected_resource = resource;
            }
        }
        selected_resource->c() += job->ProcT();
        selected_resource->jobs().push_back(job);
    }
}
